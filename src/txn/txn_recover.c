...
__recovery_set_oldest_timestamp(WT_RECOVERY *r)
{
    ...
    conn->txn_global.shared_vars.oldest_timestamp = oldest_timestamp;
    ...
    __wt_verbose_multi(session, WT_VERB_RECOVERY_ALL, "Set global oldest timestamp: %s",
      __wt_timestamp_to_string(conn->txn_global.shared_vars.oldest_timestamp, ts_string));
...
__recovery_txn_setup_initial_state(WT_SESSION_IMPL *session, WT_RECOVERY *r)
{
    ...
    WT_ASSERT(session,
      conn->txn_global.has_stable_timestamp == false &&
        conn->txn_global.shared_vars.stable_timestamp == WT_TS_NONE);

    /* Set the stable timestamp from recovery timestamp. */
    conn->txn_global.shared_vars.stable_timestamp = conn->txn_global.recovery_timestamp;
    if (conn->txn_global.shared_vars.stable_timestamp != WT_TS_NONE)
...
__wt_txn_recover(WT_SESSION_IMPL *session, const char *cfg[])
{
    ...
    if (hs_exists && !F_ISSET(conn, WT_CONN_READONLY)) {
        ...
        __wt_verbose_multi(session,
          ...
          "oldest_timestamp=%s",
          __wt_timestamp_to_string(conn->txn_global.shared_vars.stable_timestamp, ts_string[0]),
          __wt_timestamp_to_string(conn->txn_global.shared_vars.oldest_timestamp, ts_string[1]));
        rts_executed = true;

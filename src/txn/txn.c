...
__txn_remove_from_global_table(WT_SESSION_IMPL *session)
{
    ...
    WT_ASSERT(session, !WT_TXNID_LT(txn->id, WT_SHARED_VAR(txn_global, last_running)));
...
__wt_txn_active(WT_SESSION_IMPL *session, uint64_t txnid)
{
    ...
    oldest_id = WT_SHARED_VAR(txn_global, oldest_id);
...
__txn_get_snapshot_int(WT_SESSION_IMPL *session, bool publish)
{
    ...
    current_id = pinned_id = WT_SHARED_VAR(txn_global, current);
    prev_oldest_id = WT_SHARED_VAR(txn_global, oldest_id);
     ...
     */
    if ((id = WT_SHARED_VAR(txn_global, checkpoint_txn_shared).id) != WT_TXN_NONE) {
        if (txn->id != id)
            txn->snapshot[n++] = id;
        if (publish)
    ...
    if (prev_oldest_id == current_id) {
        ...
        WT_ASSERT(session, prev_oldest_id == WT_SHARED_VAR(txn_global, oldest_id));
        goto done;
    }
  ...
  uint64_t *metadata_pinnedp, WT_SESSION_IMPL **oldest_sessionp)
{
    ...
    prev_oldest_id = WT_SHARED_VAR(txn_global, oldest_id);
    last_running = oldest_id = WT_SHARED_VAR(txn_global, current);
    if ((metadata_pinned = WT_SHARED_VAR(txn_global, checkpoint_txn_shared).id) == WT_TXN_NONE)
...
__wt_txn_update_oldest(WT_SESSION_IMPL *session, uint32_t flags)
{
    ...
    current_id = last_running = metadata_pinned = WT_SHARED_VAR(txn_global, current);
    prev_last_running = WT_SHARED_VAR(txn_global, last_running);
    prev_metadata_pinned = WT_SHARED_VAR(txn_global, metadata_pinned);
    prev_oldest_id = WT_SHARED_VAR(txn_global, oldest_id);
     ...
     */
    if (WT_TXNID_LE(oldest_id, WT_SHARED_VAR(txn_global, oldest_id)) &&
      WT_TXNID_LE(last_running, WT_SHARED_VAR(txn_global, last_running)) &&
      WT_TXNID_LE(metadata_pinned, WT_SHARED_VAR(txn_global, metadata_pinned)))
        goto done;
    ...
    if (WT_TXNID_LT(WT_SHARED_VAR(txn_global, metadata_pinned), metadata_pinned))
        WT_SHARED_VAR(txn_global, metadata_pinned) = metadata_pinned;
    if (WT_TXNID_LT(WT_SHARED_VAR(txn_global, oldest_id), oldest_id))
        WT_SHARED_VAR(txn_global, oldest_id) = oldest_id;
    if (WT_TXNID_LT(WT_SHARED_VAR(txn_global, last_running), last_running)) {
        WT_SHARED_VAR(txn_global, last_running) = last_running;
...
__wt_txn_release(WT_SESSION_IMPL *session)
{
    ...
    if (WT_SESSION_IS_CHECKPOINT(session)) {
        ...
        txn->id = WT_SHARED_VAR(txn_global, checkpoint_txn_shared).id = WT_TXN_NONE;
...
__wt_txn_commit(WT_SESSION_IMPL *session, const char *cfg[])
{
    ...
    if (candidate_durable_timestamp != WT_TS_NONE) {
        prev_durable_timestamp = WT_SHARED_VAR(txn_global, durable_timestamp);
    ...
    if (update_durable_ts)
        while (candidate_durable_timestamp > prev_durable_timestamp) {
            if (__wt_atomic_cas64(&WT_SHARED_VAR(txn_global, durable_timestamp),
     ...
     * transaction.
     */
    if (prepare && txn->durable_timestamp <= WT_SHARED_VAR(txn_global, stable_timestamp)) {
...
__wt_txn_stats_update(WT_SESSION_IMPL *session)
{
    ...
    durable_timestamp = WT_SHARED_VAR(txn_global, durable_timestamp);
    pinned_timestamp = WT_SHARED_VAR(txn_global, pinned_timestamp);
    ...
    WT_STAT_SET(session, stats, txn_pinned_timestamp_oldest,
      durable_timestamp - WT_SHARED_VAR(txn_global, oldest_timestamp));
    ...
    WT_STAT_SET(session, stats, txn_pinned_checkpoint_range,
      checkpoint_pinned == WT_TXN_NONE ? 0 :
                                         WT_SHARED_VAR(txn_global, current) - checkpoint_pinned);
...
__wt_txn_global_init(WT_SESSION_IMPL *session, const char *cfg[])
{
    ...
    txn_global = &conn->txn_global;
    WT_SHARED_VAR(txn_global, current) = WT_SHARED_VAR(txn_global, last_running) = WT_SHARED_VAR(
      txn_global, metadata_pinned) = WT_SHARED_VAR(txn_global, oldest_id) = WT_TXN_FIRST;
    ...
    if (!F_ISSET(conn, WT_CONN_IN_MEMORY | WT_CONN_READONLY | WT_CONN_PANIC)) {
        ...
        if (use_timestamp) {
            __wt_verbose(session, WT_VERB_RTS,
              ...
              __wt_timestamp_to_string(conn->txn_global.shared_vars.stable_timestamp, ts_string));
...
__wt_txn_is_blocking(WT_SESSION_IMPL *session)
{
    ...
    txn = session->txn;
    txn_shared = WT_SESSION_TXN_SHARED(session);
    global_oldest = S2C(session)->txn_global.shared_vars.oldest_id;
...
__wt_verbose_dump_txn(WT_SESSION_IMPL *session)
{
    ...
    WT_RET(__wt_msg(session, "current ID: %" PRIu64, WT_SHARED_VAR(txn_global, current)));
    WT_RET(__wt_msg(session, "last running ID: %" PRIu64, WT_SHARED_VAR(txn_global, last_running)));
    WT_RET(__wt_msg(
      session, "metadata_pinned ID: %" PRIu64, WT_SHARED_VAR(txn_global, metadata_pinned)));
    WT_RET(__wt_msg(session, "oldest ID: %" PRIu64, WT_SHARED_VAR(txn_global, oldest_id)));

    WT_RET(__wt_msg(session, "durable timestamp: %s",
      __wt_timestamp_to_string(WT_SHARED_VAR(txn_global, durable_timestamp), ts_string)));
    WT_RET(__wt_msg(session, "oldest timestamp: %s",
      __wt_timestamp_to_string(WT_SHARED_VAR(txn_global, oldest_timestamp), ts_string)));
    WT_RET(__wt_msg(session, "pinned timestamp: %s",
      __wt_timestamp_to_string(WT_SHARED_VAR(txn_global, pinned_timestamp), ts_string)));
    WT_RET(__wt_msg(session, "stable timestamp: %s",
      __wt_timestamp_to_string(WT_SHARED_VAR(txn_global, stable_timestamp), ts_string)));
    WT_RET(__wt_msg(
    ...
    WT_RET(__wt_msg(session, "checkpoint running: %s",
      WT_SHARED_VAR(txn_global, checkpoint_running) ? "yes" : "no"));
    WT_RET(
    ...
    WT_RET(__wt_msg(session, "checkpoint pinned ID: %" PRIu64,
      WT_SHARED_VAR(txn_global, checkpoint_txn_shared).pinned_id));
    WT_RET(__wt_msg(
      session, "checkpoint txn ID: %" PRIu64, WT_SHARED_VAR(txn_global, checkpoint_txn_shared).id));

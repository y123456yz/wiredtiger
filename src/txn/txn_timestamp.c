...
__wt_txn_get_pinned_timestamp(WT_SESSION_IMPL *session, wt_timestamp_t *tsp, uint32_t flags)
{
    ...
    tmp_ts = include_oldest ? WT_SHARED_VAR(txn_global, oldest_timestamp) : WT_TS_NONE;
...
__txn_global_query_timestamp(WT_SESSION_IMPL *session, wt_timestamp_t *tsp, const char *cfg[])
{
    ...
    if (WT_STRING_MATCH("all_durable", cval.str, cval.len)) {
        ...
        ts = WT_SHARED_VAR(txn_global, durable_timestamp);
        ...
        WT_BARRIER();
        ts = WT_SHARED_VAR(txn_global, last_ckpt_timestamp);
    } else if (WT_STRING_MATCH("oldest_timestamp", cval.str, cval.len) ||
      WT_STRING_MATCH("oldest", cval.str, cval.len)) {
        ts = txn_global->has_oldest_timestamp ? WT_SHARED_VAR(txn_global, oldest_timestamp) : 0;
      ...
      WT_STRING_MATCH("stable", cval.str, cval.len)) {
        ts = txn_global->has_stable_timestamp ? WT_SHARED_VAR(txn_global, stable_timestamp) : 0;
    } else
...
__wt_txn_update_pinned_timestamp(WT_SESSION_IMPL *session, bool force)
{
    ...
    if (txn_global->has_pinned_timestamp && !force) {
        last_pinned_timestamp = WT_SHARED_VAR(txn_global, pinned_timestamp);
    ...
    if (pinned_timestamp != 0 &&
      (!txn_global->has_pinned_timestamp || force ||
        WT_SHARED_VAR(txn_global, pinned_timestamp) < pinned_timestamp)) {
        WT_SHARED_VAR(txn_global, pinned_timestamp) = pinned_timestamp;
        txn_global->has_pinned_timestamp = true;
        txn_global->oldest_is_pinned = WT_SHARED_VAR(txn_global, pinned_timestamp) ==
          WT_SHARED_VAR(txn_global, oldest_timestamp);
        txn_global->stable_is_pinned = WT_SHARED_VAR(txn_global, pinned_timestamp) ==
...
__wt_txn_global_set_timestamp(WT_SESSION_IMPL *session, const char *cfg[])
{
    ...
    last_oldest_ts = WT_SHARED_VAR(txn_global, oldest_timestamp);
    last_stable_ts = WT_SHARED_VAR(txn_global, stable_timestamp);
...
set:
     ...
     */
    if (has_durable) {
        WT_SHARED_VAR(txn_global, durable_timestamp) = durable_ts;
    ...
    if (has_oldest &&
      (!txn_global->has_oldest_timestamp || force ||
        oldest_ts > WT_SHARED_VAR(txn_global, oldest_timestamp))) {
        WT_SHARED_VAR(txn_global, oldest_timestamp) = oldest_ts;
    ...
    if (has_stable &&
      (!txn_global->has_stable_timestamp || force ||
        stable_ts > WT_SHARED_VAR(txn_global, stable_timestamp))) {
        WT_SHARED_VAR(txn_global, stable_timestamp) = stable_ts;
...
__txn_assert_after_reads(WT_SESSION_IMPL *session, const char *op, wt_timestamp_t ts)
{
    ...
    if (EXTRA_DIAGNOSTICS_ENABLED(session, WT_DIAGNOSTIC_TXN_VISIBILITY)) {
        txn_global = &S2C(session)->txn_global;
        WT_ORDERED_READ(session_cnt, WT_SHARED_VAR(S2C(session), session_cnt));
        WT_STAT_CONN_INCR(session, txn_walk_sessions);
...
__wt_txn_set_read_timestamp(WT_SESSION_IMPL *session, wt_timestamp_t read_ts)
{
    ...
    ts_oldest = WT_SHARED_VAR(txn_global, oldest_timestamp);
...
__wt_txn_clear_read_timestamp(WT_SESSION_IMPL *session)
{
    ...
    if (F_ISSET(txn, WT_TXN_SHARED_TS_READ)) {
        ...
        WT_ASSERT(session,
          txn_shared->read_timestamp >= S2C(session)->txn_global.shared_vars.pinned_timestamp);

    ...
    case WT_SYNC_CHECKPOINT:
        ...
        if (!btree->modified && !F_ISSET(conn, WT_CONN_RECOVERING | WT_CONN_CLOSING_CHECKPOINT) &&
          (btree->rec_max_txn >= txn->snap_min ||
            (conn->txn_global.checkpoint_timestamp_shared !=
                conn->txn_global.last_ckpt_timestamp_shared &&
              btree->rec_max_timestamp > conn->txn_global.checkpoint_timestamp_shared)))
            __wt_tree_modify_set(session);
        break;
    case WT_SYNC_CLOSE:

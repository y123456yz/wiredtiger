...
__wt_txn_oldest_id(WT_SESSION_IMPL *session)
{
    ...
    if (session->dhandle != NULL && WT_IS_METADATA(session->dhandle))
        return (txn_global->metadata_pinned_shared);
     ...
     */
    WT_ORDERED_READ(oldest_id, txn_global->oldest_id_shared);
...
__wt_txn_pinned_timestamp(WT_SESSION_IMPL *session, wt_timestamp_t *pinned_tsp)
{
    ...
    *pinned_tsp = pinned_ts = txn_global->pinned_timestamp_shared;
     ...
     */
    WT_READ_BARRIER();
    checkpoint_ts = txn_global->checkpoint_timestamp_shared;
...
__wt_txn_id_alloc(WT_SESSION_IMPL *session, bool publish)
{
    ...
    if (publish) {
        ...
        WT_PUBLISH(txn_shared->id, txn_global->current_shared);
        id = __wt_atomic_addv64(&txn_global->current_shared, 1) - 1;
        session->txn->id = id;
...
__wt_txn_cursor_op(WT_SESSION_IMPL *session)
{
    ...
    if (txn->isolation == WT_ISO_READ_UNCOMMITTED) {
        if (txn_shared->pinned_id == WT_TXN_NONE)
            txn_shared->pinned_id = txn_global->last_running_shared;
...
__wt_txn_activity_check(WT_SESSION_IMPL *session, bool *txn_active)
{
    ...
    *txn_active = (txn_global->oldest_id_shared != txn_global->current_shared ||
      txn_global->metadata_pinned_shared != txn_global->current_shared);

 ...
 *     WT_SESSION.compact method.
 mongo server执行compact命令或者compact后台线程执行__background_compact_server操作会走到这里
 */
int
__wti_session_compact(WT_SESSION *wt_session, const char *uri, const char *config)
{
    ...
    printf("yang test ......__wti_session_compact..........config:%s, uri:%s\r\n", config, uri);
    /* Trigger the background server. */
    //后台compact走这里，注意第一次带backgroud=ture进入该流程，在__wt_background_compact_signal中使能compact线程后会去掉background=true配置
    //  这时候就会进入下面的else分支进行真正的compact操作
    if ((ret = __wt_config_getones(session, config, "background", &cval) == 0)) {
        ...
        //通知compact后台线程__background_compact_server
    ...
    printf("yang test ......__wti_session_compact session->compact->file_count:%u, op_handle_next:%u\r\n", 
